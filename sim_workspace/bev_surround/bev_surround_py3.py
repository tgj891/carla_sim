...
#read car
tesla_car = cv2.imread(f"data/tesla_car.png")
...
def Init_Params():
    #read homography
    f_H = np.load(f"data/front_homography.npy")
    calib_H_param["front"] = f_H
    b_H = np.load(f"data/back_homography.npy")
    calib_H_param["back"] = b_H
    l_H = np.load(f"data/left_homography.npy")
    calib_H_param["left"] = l_H
    r_H = np.load(f"data/right_homography.npy")
